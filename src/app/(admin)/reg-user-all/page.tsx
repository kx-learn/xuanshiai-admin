...
export default function RegUserAllPage() {
  ...
  const addValid = addForm.nickname.trim().length >= 1 && addForm.phone.trim().length >= 1 && addForm.password.length >= 8;
  ...
  return <div>
    ...
    {addOpen && <div className="fixed inset-0 z-50 flex bg-black/35" onClick={closeAdd}><div role="dialog" aria-modal="true" className="reg-add-drawer-panel ml-auto flex h-full w-[460px] flex-col bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between border-b border-[#edf0f5] px-6 py-4"><h2 className="text-base font-medium text-[#25272d]">添加账号</h2><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={closeAdd}>取消</Button><Button variant="primary" size="sm" disabled={!addValid || addSaving} onClick={() => void submitAdd()}>{addSaving ? "提交中…" : "确定提交"}</Button></div></div><div className="flex-1 overflow-y-auto px-6 py-5"><div className="space-y-4"><label className="block text-sm text-[#25272d]"><span className="mr-1 text-[#ff4d4f]">*</span>昵称<input value={addForm.nickname} onChange={(event) => setAddForm({ ...addForm, nickname: event.target.value })} placeholder="3-28字符，最多14个汉字" maxLength={28} className="mt-2 h-9 w-full rounded border border-[#d9dde6] px-3 text-sm focus:border-[#3658f7] focus:outline-none" /></label><label className="block text-sm text-[#25272d]"><span className="mr-1 text-[#ff4d4f]">*</span>手机<input value={addForm.phone} onChange={(event) => setAddForm({ ...addForm, phone: event.target.value })} placeholder="请输入手机号" className="mt-2 h-9 w-full rounded border border-[#d9dde6] px-3 text-sm focus:border-[#3658f7] focus:outline-none" /></label><label className="block text-sm text-[#25272d]"><span className="mr-1 text-[#ff4d4f]">*</span>密码<div className="relative mt-2"><input type={showAddPwd ? "text" : "password"} value={addForm.password} onChange={(event) => setAddForm({ ...addForm, password: event.target.value })} placeholder="请输入密码（至少8位）" className="h-9 w-full rounded border border-[#d9dde6] px-3 pr-9 text-sm focus:border-[#3658f7] focus:outline-none" /><button type="button" aria-label={showAddPwd ? "隐藏密码" : "显示密码"} onClick={() => setShowAddPwd((current) => !current)} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#9aa3b2] hover:text-[#3658f7]">{showAddPwd ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></label><div className="text-sm text-[#25272d]"><span>头像</span><div className="mt-2 flex flex-wrap gap-2"><button type="button" className="flex items-center gap-1.5 rounded border border-[#d9dde6] bg-white px-4 py-2 text-sm text-[#3658f7] hover:border-[#3658f7]"><span>↑</span>上传图片</button><button type="button" className="flex items-center gap-1.5 rounded border border-[#d9dde6] bg-white px-4 py-2 text-sm text-[#3658f7] hover:border-[#3658f7]"><span>🖼</span>从云端素材选择</button></div></div></div></div></div></div>}
