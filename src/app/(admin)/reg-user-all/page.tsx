...
type Dialog = "create" | "edit" | "status" | "password" | "delete" | null;
...
type AddForm = { nickname: string; username: string; password: string };
const initialAddForm: AddForm = { nickname: "", username: "", password: "" };
...
export default function RegUserAllPage() {
  ...
  const openDelete = (account: AdminAccountItem) => {
    setSelected(account);
    setError("");
    setDialog("delete");
  };
  const submit = async () => {
    ...
    try {
      ...
      if (dialog === "delete" && selected) await adminEndpoints.deleteAdminAccount(selected.id);
      const deleted = dialog === "delete";
      close(); await load(deleted && result.items.length === 1 && result.page > 1 ? result.page - 1 : result.page);
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : "操作失败";
      if (dialog === "delete" && /405|404|Method Not Allowed|Not Found/i.test(message)) setError(`删除接口暂不可用（DELETE /api/v1/admin/matchmaker/accounts/{id}）：${message}`);
      else setError(message);
    }
    finally { setSaving(false); }
  };
  const addValid = addForm.nickname.trim().length >= 1 && addForm.username.trim().length >= 1 && addForm.password.length >= 8;
  const submitAdd = async () => {
    ...
    try {
      await adminEndpoints.createAdminAccount({
        ...
        display_name: addForm.nickname.trim(),
        matchmaker_user_id: null,
        data_scope: "SELF",
        organization_id: null,
        permissions: [],
      });
      setAddOpen(false); setAddForm(initialAddForm); setShowAddPwd(false);
      await load(result.page);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "添加账号失败"); }
    finally { setAddSaving(false); }
  };
  const closeAdd = () => { setAddOpen(false); setAddForm(initialAddForm); setShowAddPwd(false); };
  ...
  const valid = dialog === "create" ? Boolean(form.username.trim() && form.display_name.trim() && form.password.length >= 8) : dialog === "edit" ? Boolean(form.display_name.trim()) : dialog === "status" ? Boolean(reason.trim()) : dialog === "password" ? Boolean(reason.trim() && form.password.length >= 8) : dialog === "delete" ? Boolean(selected) : false;
